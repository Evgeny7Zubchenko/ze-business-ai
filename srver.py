...
def get_site(sid):
    ...
    cur = c.cursor()
    cur.execute(
        "SELECT id,title,html,product_pack,ad_pack,created_at,updated_at FROM sites WHERE id=?",
        (sid,)
    )
    row = cur.fetchone()
    c.close()
    ...
    if acc["plan"] == "free":
        return JSONResponse(
            ...
            content={
                ...
                "reply": "AI-редактор доступен в Premium."
            }
        )
...
def product_pack(req: ProductPackRequest):
    ...
    if acc["plan"] == "free":
        return JSONResponse(
            ...
            content={
                "error": "premium_required",
                "reply": "Product Pack доступен в Premium."
            }
        )
...
def ad_pack(req: AdPackRequest):
    ...
    if acc["plan"] == "free":
        return JSONResponse(
            ...
            content={
                "error": "premium_required",
                "reply": "Рекламный пакет доступен в Premium."
            }
        )
...
def business_pack(req: BusinessPackRequest):
    ...
    if acc["plan"] == "free":
        return JSONResponse(
            ...
            content={
                "error": "premium_required",
                "reply": "Создание бизнеса доступно в Premium."
            }
        )
